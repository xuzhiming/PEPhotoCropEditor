...
//        self.showsGridMinor = NO;
        ...
        NSString *borderPath = [[PEBundleUtils bundle] pathForResource:@"PEPhotoCropEditorBorder" ofType:@"png"];
        imageView.image =[[UIImage imageWithContentsOfFile:borderPath] resizableImageWithCapInsets:UIEdgeInsetsMake(23.0f, 23.0f, 23.0f, 23.0f)];
...
- (void)setKeepingAspectRatio:(BOOL)keepingAspectRatio
{
    ...
    if (self.keepingAspectRatio) {
        CGFloat width = CGRectGetWidth(self.bounds);
        CGFloat height = CGRectGetHeight(self.bounds);
        self.fixedAspectRatio = 1.f;//fminf(width / height, height / width);
    }
}
